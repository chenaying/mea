    ...
    if args.using_hard_prompt:
        if args.use_memory and MEA_CAP_AVAILABLE:
            ...
            try:
                ...
                print(f"Retrieved {len(select_memory_captions)} memory captions")
                ...
                print("Falling back to ViECap original method...")
                # 回退到原始方法
                if entities_text is None or texts_embeddings is None:
                    print("Error: Cannot fallback, entity vocabulary not loaded!")
                    return
            ...
            # ========== ViECap 原始方法 ==========
            if entities_text is None or texts_embeddings is None:
                print("Error: Entity vocabulary not loaded!")
                return
            logits = image_text_simiarlity(texts_embeddings, temperature = args.temperature, images_features = image_features)
