...
def validation_nocaps(
    ...
    parser_model = None,                     # MeaCap: scene graph parser
    parser_tokenizer = None,                 # MeaCap: parser tokenizer
    wte_model = None,                        # MeaCap: SentenceBERT
    vl_model = None,                         # MeaCap: CLIP for retrieval
    memory_clip_embeddings = None,           # MeaCap: memory embeddings
    memory_captions = None,                  # MeaCap: memory captions
    retrieve_on_CPU = False                 # MeaCap: whether to retrieve on CPU
) -> None:
        ...
        if args.using_hard_prompt:
            if args.use_memory and MEA_CAP_AVAILABLE and parser_model is not None:
                ...
                try:
                    # 1. 检索记忆库
                    if retrieve_on_CPU:
                        image_features_cpu = image_features.to('cpu')
                    ...
                    if entities_text is not None and texts_embeddings is not None:
                        ...
                        print(f"Error: Cannot retrieve concepts for image {image_id}")
                        continue
            else:
                # ========== ViECap 原始方法 ==========
                if entities_text is None or texts_embeddings is None:
                    print(f"Error: Entity vocabulary not loaded for image {image_id}")
                    continue
                logits = image_text_simiarlity(texts_embeddings, temperature = args.temperature, images_features = image_features)
    ...
    if args.name_of_datasets == 'nocaps': # nocaps
        if args.using_image_features:
            validation_nocaps(args, inpath, entities_text, texts_embeddings, model, tokenizer,
                            parser_model=parser_model, parser_tokenizer=parser_tokenizer,
                            wte_model=wte_model, vl_model=vl_model,
                            memory_clip_embeddings=memory_clip_embeddings, memory_captions=memory_captions,
